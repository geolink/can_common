...
CAN_COMMON::CAN_COMMON(int numFilt)
{
  ...
  memset(cbCANFrame, 0, sizeof(cbCANFrame));
  cbGeneral = 0;
  enablePin = 255;
...
{
  if (id > 0x7FF)
    return setRXFilter(id, 0x1FFFFFFF, true);
  else
    return setRXFilter(id, 0x7FF, false);
}
...
{
  if (id > 0x7FF)
    return setRXFilter(id, mask, true);
  else
    return setRXFilter(id, mask, false);
}
